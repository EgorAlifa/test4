...
module.exports = {
    meta: {
        ...
        messages: {
            ...
            notMatch:
                "The 'lang' attr not match file extension '.{{allows}}'. ",
            unexpectedDefault:
    ...
    create(context) {
        ...
        function verify(element) {
            ...
            const src = utils.getAttribute(element, 'src')
            ...
            if (src != null
                && (
                    (lang != null
                        && src.value.value.endsWith(`.${lang.value.value}`) === false)
                    || (lang == null
                        && src.value.value.endsWith(`.${DEFAULT_LANGUAGES[tag][0]}`) === false)
                )
            )
            {
                messageId = 'notMatch'
                context.report({
                    node: lang != null ? lang : src,
                    messageId,
                    data: {
                        allows: src.value.value.split('.').pop()
                    }
                })

            }
            else if (lang != null && !option.lang.has(lang.value.value)) {
                if (!option.allowNoLang) {
                    messageId = 'expected'
