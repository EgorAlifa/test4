...
class CSSSelectorResolver {
    ...
    resolveNodesSelectors(nodes, parentSelector) {
        ...
        for (const node of nodes) {
            ...
            if (css_nodes_1.hasSelectorNodes(node)) {
    ...
    resolveNestingSelectors(owner, selectorNodes, parentSelectors, container) {
        if (selectors_1.isNestingAtRule(container)) {
            return this.resolveSelectorForNestContaining(owner, selectorNodes, selectors_1.findNestingSelector(selectorNodes), parentSelectors, container);
        }
    ...
    resolveSelectorForNestPrefixed(owner, selectorNodes, parentSelectors, container) {
        if (!selectorNodes.length || !selectors_1.isNestingSelector(selectorNodes[0])) {
            return [new ResolvedSelector(owner, selectorNodes)];
        }
    ...
    resolveSelectorForNestConcat(owner, selectorNodes, parentSelectors, _container) {
        if (!parentSelectors) {
            ...
            if (selectors_1.isDescendantCombinator(nodes[0])) {
                nodes.shift();
            }
            if (selectors_1.isDescendantCombinator(nodes[nodes.length - 1])) {
                nodes.pop();
            }
        ...
        return parentSelectors.selectors.map((parentSelector) => {
            ...
            if (parent.length > 0 &&
                selectors_1.isSelectorCombinator(parent[parent.length - 1]) &&
                nodes.length > 0 &&
                selectors_1.isSelectorCombinator(nodes[0])) {
                if (selectors_1.isDescendantCombinator(nodes[0])) {
                    nodes.shift();
                }
                else if (selectors_1.isDescendantCombinator(parent[parent.length - 1])) {
                    parent.pop();
                }
            }
    ...
    resolveSelectorForNestContaining(owner, selectorNodes, nestingInfo, parentSelectors, _container) {
        ...
        const maybeJoinLeft = nestingLeftNode &&
            ...
            !selectors_1.isSelectorCombinator(nestingLeftNode);
        const maybeJoinRight = nestingRightNode &&
            nestingNode.range[1] === nestingRightNode.range[0] &&
            selectors_1.isTypeSelector(nestingRightNode);
        let resolved;
        if (parentSelectors) {
            resolved = parentSelectors.selectors.map((p) => {
                ...
                const needJoinLeft = maybeJoinLeft && selectors_1.isTypeSelector(parentSelector[0]);
                const needJoinRight = maybeJoinRight &&
                    !selectors_1.isSelectorCombinator(parentSelector[parentSelector.length - 1]);
                if (needJoinLeft &&
                    needJoinRight &&
            ...
            const before = [...beforeSelector];
            const after = [...afterSelector];
            while (selectors_1.isDescendantCombinator(before[0])) {
                before.shift();
            }
            if (selectors_1.isDescendantCombinator(after[0])) {
                while (selectors_1.isDescendantCombinator(before[before.length - 1])) {
                    before.pop();
                }
            }
            while (before.length === 0 && selectors_1.isDescendantCombinator(after[0])) {
                after.shift();
            }
            while (selectors_1.isDescendantCombinator(after[after.length - 1])) {
                after.pop();
            }
