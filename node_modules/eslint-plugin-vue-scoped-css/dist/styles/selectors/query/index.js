...
function* queryStep(elements, selectorNode, document) {
    ...
    else if (selectors_1.isVDeepPseudo(selectorNode)) {
        yield* genVDeepElements(elements, selectors_1.normalizePseudoParams(selectorNode, selectorNode.nodes), query);
        return;
    }
    else if (selectors_1.isVSlottedPseudo(selectorNode)) {
        yield* genVSlottedElements(elements, selectors_1.normalizePseudoParams(selectorNode, selectorNode.nodes), query);
        return;
    }
    else if (selectors_1.isVGlobalPseudo(selectorNode)) {
        yield* genVGlobalElements(elements, selectors_1.normalizePseudoParams(selectorNode, selectorNode.nodes), document, query);
        return;
    }
...
function* genChildElements(elements) {
    ...
    for (const element of elements) {
        if (elements_1.isSlotElement(element)) {
            continue;
        }
...
function matchTransitionClassName(element, className, document) {
    const classBases = templates_1.isTransitionElement(element)
