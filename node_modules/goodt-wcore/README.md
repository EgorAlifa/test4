...
├── .utils           # nodejs scripts utils for configs, sandbox mode and build-time scripts
├── dist             #
├── public           #
...
│   ├── sandbox      # core sandbox-mode stuff
...
```bash
npm i --no-save @goodt/config @goodt/browserslist-config @goodt/eslint-config-base @goodt/stylelint-config-base @goodt-wcore/scaffold @goodt/styleguide
```
...
class ExampleApiService extends BaseApiService {
   ...
   */
    async getItemById(itemId) {
         ...
         * // ApiServiceErrorCode.UNKNOWN |       - сломалось что-то при работе клиента
         * // ApiServiceErrorCode.CONFLICT |      - сервер не смог обработать запрос из-за конфликта с текущим состоянием ресурса
         */
