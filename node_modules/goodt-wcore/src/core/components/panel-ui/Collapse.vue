<template>
    <ui-collapse class="collapse pos-rel radius" :visible="open">
        <template #heading="{ isOpen, toggle }">
            <div
                class="collapse-header radius pad-v-2 pad-h-l1 cursor-pointer bg-white u-select-none"
                :class="{ open: isOpen }"
                @click="onClick({ toggle, isOpen })">
                <slot name="heading" v-bind="{ toggle, isOpen }">
                    <div class="row row-collapse">
                        <div class="col col-vmid text-small text-truncate">
                            <!--
                            @slot header slot
                            -->
                            <slot name="header" v-bind="{ isOpen, toggle }">
                                <ui-label :prop="prop">
                                    <template #default="{ getLabel }">
                                        <slot name="label" v-bind="{ getLabel }"></slot>
                                    </template>
                                    <template #hint="{ getHint }">
                                        <slot name="hint" v-bind="{ getHint }"></slot>
                                    </template>
                                </ui-label>
                            </slot>
                        </div>
                        <div class="col col-auto col-vmid">
                            <div class="icon w-auto pull-left">
                                <i class="mdi mdi-chevron-right" :class="{ 'mdi-rotate-90': isOpen }" />
                            </div>
                        </div>
                    </div>
                </slot>
            </div>
        </template>
        <template #default>
            <div class="pad-l1">
                <!--
                @slot content slot
                -->
                <slot />
            </div>
        </template>
    </ui-collapse>
</template>
<style lang="pcss" scoped>
.collapse {
    border: 1px solid var(--color-grey-light);
    &-header {
        overflow: hidden;
        &:before {
            position: absolute;
            width: 100%;
            height: 100%;
            top: 0;
            left: 0;
            opacity: 0;
            background: var(--color-primary-focus);
            transition: all 0.2s ease;
            content: '';
        }
        &.open {
            position: sticky;
            top: 0;
            z-index: 1;
            color: var(--color-primary);
            &:before {
                opacity: 1;
            }
        }
    }
}
</style>
<script>
import { Collapse as UiCollapse } from 'goodteditor-ui';
import UiLabel from './Label.vue';


export default {
    components: { UiCollapse, UiLabel },
    props: {
        /**
         * Whether the collapse is 'open'
         */
        open: {
            type: Boolean,
            default: false
        },
        /**
         * prop name
         * @type {import('vue').PropOptions<string>}
         */
        prop: {
            type: String,
            default: ''
        }
    },
    methods: {
        onClick({ toggle, isOpen }) {
            toggle();
            this.$emit('toggle', !isOpen);
            this.$emit('update:open', !isOpen);
        }
    }
};
</script>
