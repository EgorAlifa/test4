<template>
    <div class="d-flex flex-col w-100 h-100">
        <div class="flex-grow scroll-x scroll-y">
            <ui-excel-table v-bind="{ header, body: rows }">
                <template v-if="selectionEnabled" #header-cell-summary>
                    <th>
                        <label class="checkbox">
                            <input type="checkbox" :checked="headerAllSelected" @change="onColumnSummarySelect" />
                            <i />
                        </label>
                    </th>
                </template>
                    ...
                    <th
                        ...
                        <div class="text-center">
                            <div v-if="selectionEnabled && item" class="mar-bot-2">
                                <label class="checkbox">
                                    <input type="checkbox" :checked="item.selected" @change="onColumnSelect(item)" />
                                    <i />
                                </label>
                            </div>
                            <div>{{ letter }}</div>
                        ...
                        <div v-if="selectionEnabled && item && !item.selected" class="shim events-none">
                ...
                <div class="shim-content h-100 scroll-y">
                    ...
                    <div v-if="error" class="alert alert-error mar-l5 pull-center">
                        ...
                        <div class="alert-body text-small">
                            ...
                            <div>
                                <div class="btn btn-error btn-small" @click.prevent="errorCollapsed = !errorCollapsed">
                                    <template v-if="errorCollapsed">Показать</template>
                                    <template v-else>Скрыть</template>
                                    подробное описание
                                </div>
                            </div>
                            <div v-if="!errorCollapsed" class="text-xsmall mar-top-l1" style="white-space: pre-line">
                                {{ error.toDebugString() }}
                            </div>
                        </div>
                    </div>
        ...
        <div class="pad-l1 text-xsmall">
            <div v-show="datasetName" class="row row-collapse">
                <div class="col">
                    <div>
                        <span class="color-primary cursor-pointer" title="Обновить" @click="load(page)">
                            {{ datasetName }}
                        </span>
                    </div>
                    <div>
                ...
                <div class="col col-auto col-vmid">
                    <ui-pagination v-if="pages > 1" v-bind="{ page, pages }" @select="(page) => load(page)" />
                </div>
            </div>
        </div>
        ...
        <div v-if="showDebug" class="popup" style="z-index: 10" @click="showDebug = false">
            <div class="popup-dialog w-11-12" style="max-width: 800px" @click.stop>
                ...
                <div class="popup-dialog-body">
                    ...
                    <code>query-dremio</code>
                    <div class="pos-rel">
                        <pre class="text-xsmall p" style="max-height: 35vh">{{ dremio }}</pre>
                        <div
                            class="btn btn-ghost btn-icon btn-small pos-abs pos-bot-right"
                    ...
                    <div class="pos-rel">
                        <pre class="text-xsmall" style="white-space: pre-line; max-height: 35vh">{{ datasetSql }}</pre>
                        <div
                            class="btn btn-ghost btn-icon btn-small pos-abs pos-bot-right"
                            @click="writeToClipboard(datasetSql)">
                            <i class="mdi mdi-content-copy"></i>
                        </div>
                    </div>
                </div>
        ...
        <ui-portal>
            <div class="toast pos-fixed pos-bot-center mar-bot-l1" v-if="toastMessage">{{ toastMessage }}</div>
        </ui-portal>
    </div>
</template>
<script>
...
import { Query, SDKFactory } from '@goodt-common/dremio';
import UiExcelTable from './ExcelTable.vue';
...
export default {
    ...
    computed: {
        ...
        headerAllSelected() {
            return this.header.reduce((acc, { selected }) => (selected ? acc + 1 : acc), 0) === this.header.length;
        },
        queryStr() {
            if (!this.query) {
                return null;
    ...
    watch: {
        ...
        dataProviderId: {
            handler(dataProviderId) {
                this.dremioSdk = SDKFactory({ admin: true }, { dataProviderId });
                this.load();
            },
            immediate: true
        }
    ...
    methods: {
        ...
        onCopy() {
            this.showToast('Скопировано в буфер');
        },
        showToast(msg) {
            if (this.toastDispose) {
                this.toastDispose();
            }
            this.toastMessage = msg;
            this.toastDispose = delay(() => {
