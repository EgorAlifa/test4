    ...
    <div>
        <div class="p">
            <div class="row row-collapse">
                <div class="col">
                    <div class="form-label form-label-small">Название</div>
                    <input
                        v-model="nameEdit"
                        class="input w-100"
        ...
        <div class="form-label form-label-small">Поля</div>
        <div class="pad-3 bg-grey-lighter radius cf">
            <draggable v-if="fieldsValid" v-model="fieldsEdit" v-bind="dragOptions" @change="onFieldsChanged">
            ...
            <div v-else class="alert alert-error">
                <div class="alert-body text-small">Необходимо выбрать поля</div>
            </div>
            <div class="pos-rel pull-right" @click.stop>
                <div :id="popoverTarget" class="icon cursor-pointer color-primary" @click="isPopoverShown = true">
                    <i class="mdi mdi-plus-circle-outline mdi-18px" />
                </div>
                <ui-popover v-bind="popoverProps" :show.sync="isPopoverShown">
                    <div class="dropdown text-small">
                        <ul>
                            <li v-for="field in fieldsAvailable" :key="field" @click="onFieldAdd(field)">
                                {{ field }}
                            </li>
                        </ul>
                    </div>
...
export default {
    ...
    computed: {
        ...
        popoverTarget() {
            return `popover-target-${this.name}`;
        },
        popoverProps() {
            const { popoverTarget } = this;
            return { target: `#${popoverTarget}`, position: 'bottom-end' };
        }
    },
    watch: {
        name: {
