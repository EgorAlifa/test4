...
export const ConditionFunction = {
    ...
    [ConditionOperator.NOT_EQUALS]: (condVal, cellVal) => getStringToFloat(condVal) !== getStringToFloat(cellVal)
};

export const ConditionCompareValueType = {
    STATIC: 'static',
    FIELD: 'field'
};
...
export const ConditionLogicalOperator = {
    AND: 'and',
    OR: 'or'
};

export const CURRENT_FIELD_ALIAS = '__current__';
...
export const createCellSettings = ({
    ...
    actions: createCellActions(actions)
});
...
export const createConditionRuleItem = ({
    fieldAlias = CURRENT_FIELD_ALIAS,
    operator = ConditionOperator.MORE,
    logicalOperator = ConditionLogicalOperator.AND,
    compareType = ConditionCompareValueType.STATIC,
    comparedValue = '',
    comparedAlias = '',
...
export const createCondition = (condition = {}) => {
    if (Array.isArray(condition.conditions) === false) {
        return createLegacyCondition(condition);
    }
...
export const createConditionWithUid = (condition = createCondition()) => {
    const normalizedCondition = createCondition(condition);

    return {
        ...normalizedCondition,
        uid: uniqueId(),
        conditions: normalizedCondition.conditions.map((item) => ({
            ...item,
            uid: uniqueId()
        }))
    };
