...
function generateTableCells(
    ...
    const buildSortedPathsIndexes = ({
        ...
        for (let level = 0; level < levels; level++) {
            pathsEntries.sort(([aliasKey1, path1], [aliasKey2, path2]) => {
                ...
                if (columnOptions.isUsed) {
                    const { path, valueLevel, isDesc } = columnOptions;
                    const resultTrue = isDesc === true ? -1 : 1;
                    const resultFalse = isDesc === true ? 1 : -1;
                    const currentPath1 = path1.slice(0, level + 1);
                    const currentPath2 = path2.slice(0, level + 1);

                    const path1Val = buildCellValue({
                        ...valuesData[valueLevel],
                        equalIndexes: columnOptions.isTotal
                            ? getHeapNodeRowsByPath({ path: currentPath1, heap })
                            : resolveCellEqualIndexes({ rowPathParts: currentPath1, columnPathParts: path })
                    });

                    const path2Val = buildCellValue({
                        ...valuesData[valueLevel],
                        equalIndexes: columnOptions.isTotal
                            ? getHeapNodeRowsByPath({ path: currentPath2, heap })
                            : resolveCellEqualIndexes({ rowPathParts: currentPath2, columnPathParts: path })
                    });
    ...
    function buildHeader(columnsCollapsedPaths) {
        const isColumnPathRepeated = (columnPathParts, columnPathIndex, level) =>
            columns
                .slice(0, level + 1)
                .every(
                    (__, colLevel) =>
                        columnsCollapsedPaths[columnPathIndex - 1]?.[colLevel] === columnPathParts?.[colLevel]
                );

        const result = columns.reduce((resultRows, { title, height, dataAlias }, level) => {
            ...
            if (useDrawTypeTableView) {
                ...
                namesCells = columnsCollapsedPaths.flatMap((columnPathParts, columnPathIndex) =>
                    valuesLoopItems.map((__unusedVal, valueDataIndex) =>
                        utils.createCell({
                            ...
                            hasBeenCollapsed:
                                ...
                                !isColumnPathRepeated(columnPathParts, columnPathIndex, level)
                        })
                    )
                );
                          ...
                          .map((__unused, level, cur) => {
                              ...
                              if (rowCellType === constants.CellsTypes.TOTAL_CELL) {
                                  return utils.createCell({
                                      ...
                                      dataAlias: rows[level].dataAlias,
                                      level,
                                      hasBeenCollapsed,
