                            ...
                            <div
                                ...
                                <template v-for="(cell, index) in cells">
                                    <div
                                        ...
                                        @click="
                                            onCellClick(
                                                { ...cell, id, index, rowIndex: resolveTableRowIndex(rowIndex) },
                                                $event
                                            )
                                        "
...
export default {
    ...
    computed: {
        ...
        paginationStyle() {
            const { paginationActiveButtonSettings, paginationInactiveButtonSettings } = this.props ?? {};
            return this.$genCssVarsStyle({
                ...buildPaginationButtonCssVars(paginationActiveButtonSettings, 'pag-active-'),
                ...buildPaginationButtonCssVars(paginationInactiveButtonSettings, 'pag-inactive-')
            });
        },
        isFlat() {
        ...
        fixedFirstColumnCellsCount() {
            ...
            return 1 + (isUsedIndexes === true ? 1 : 0);
        },
    ...
    methods: {
        ...
        async fetchRows({
            ...
            const isColumnSortUsed = columnSortOptions != null && columnSortOptions.isUsed;
            const sort = isColumnSortUsed ? this.buildColumnSort(columnSortOptions, metricsCustom) : null;
            ...
            if (isColumnSortUsed === false) {
                this.appendValuesSort(request, columnSortOptions, metricsCustom);
            }
        ...
        buildSlicedRequest({ limit = null, offset = null, playerRows = this.playerRows, isFlat = this.isFlat } = {}) {
            ...
            if (!isFlat) {
                const sort = columnSortOptions.isUsed ? this.buildColumnSort(columnSortOptions, metricsCustom) : null;
        ...
        buildColumnSort(columnSortOptions, metricsCustom) {
            const { QuerySortDirection } = this;
            const sortSettings = this.playerValues[columnSortOptions.level];
            return {
                name: this.resolveSortName(sortSettings, metricsCustom),
                direction: columnSortOptions.sortDesc ? QuerySortDirection.DESC : QuerySortDirection.ASC
            };
        },
        buildRequest({
            dimensions = [],
            ...
            const sortName = sort == null ? null : sort.name;
            oldRequestSort
                .filter(({ name }) => sortableFields.has(name) && name !== sortName)
                .forEach((item) => {
                    query.sortAdd(item);
                });
        ...
        appendValuesSort(request, columnSortOptions, metricsCustom) {
            if (columnSortOptions != null && columnSortOptions.isUsed) {
                request.query.sortAdd(this.buildColumnSort(columnSortOptions, metricsCustom));
                return;
            }
            this.appendPlayerValuesSort(request);
        },
        ...
        async fetchPaginationData(rowFilter, columnSortOptions = null) {
            ...
            try {
                ...
                return this.fillPaginationRowsActionsData(result ?? [], rowFilter);
            } catch (e) {
                this.$handleError(e);
                return [];
            }
        ...
        resolveRowsActionsDataAliases(rows) {
            ...
            return uniqArray(
                rows
                    ...
                    .flatMap(({ dataAlias, actions: { link } }) =>
                        [link.url, ...link.queryMetrics].filter((alias) => alias !== dataAlias && rowAliases.has(alias))
                    )
            );
        },
        async fillPaginationRowsActionsData(rows, rowFilter) {
            ...
            if (rows.every((rowData) => actionDataAliases.every((alias) => rowData[alias] != null))) {
                return rows;
            }

            const { playerFiltersValues, dimensions } = this;
            const rowAlias = row.dataAlias;
            const rowIndex = dimensions.findIndex((dimension) => dimension === rowAlias);
            const filters = this.buildFilters(
                playerFiltersValues.map((filter, index) => (index === rowIndex ? rowFilter : filter))
            );
            const result = await this.fetchData({
                dimensions: [rowAlias, ...actionDataAliases],
                filters,
                limit: 0
            });
            const actionRows = result?.rows ?? [];
            const actionRowsByValue = actionRows.reduce((acc, actionRow) => {
                const rowValue = actionRow[rowAlias];
                if (!acc.has(rowValue)) {
                    acc.set(rowValue, {});
                }
                const actionData = acc.get(rowValue);
                actionDataAliases.forEach((alias) => {
                    if (actionData[alias] == null && actionRow[alias] != null) {
                        actionData[alias] = actionRow[alias];
                    }
                });
                return acc;
            }, new Map());
            ...
            return rows.map((rowData) => {
                const actionData = actionRowsByValue.get(rowData[rowAlias]);
                if (actionData == null) {
                    return rowData;
                }
                let result = rowData;
                actionDataAliases.forEach((alias) => {
                    if (rowData[alias] == null && actionData[alias] != null) {
                        if (result === rowData) {
                            result = { ...rowData };
                        }
                        result[alias] = actionData[alias];
                    }
                });
                return result;
            });
        ...
        async load() {
            ...
            try {
                ...
                } = await this.fetchRows({ columnSortOptions });
                const names = rowsPaths.map(([name]) => name);
                const paginationDataPromise =
                    names.length > 0 ? this.fetchPaginationData(names, columnSortOptions) : Promise.resolve([]);
        ...
        resolveExportDimensionType({ valueType }) {
        ...
        resolveExportDimensionFormatSettings(settings) {
            ...
            return {
                ...
                decimals: settings.valueType === ValueType.NUMBER ? decimals : 0
            };
        },
        composeCreateReportPivotFileds(fields) {
            ...
            return preparedFields.map((field) => {
                ...
                const { type: formattedType, decimals: formattedDecimals } =
                    this.resolveExportNumberFormatSettings(format);
                let settingsType = formattedType;
                let settingsDecimals = formattedDecimals;
                let adaptType = type;
                if (['row', 'column'].includes(adaptType)) {
                    ({ type: settingsType, decimals: settingsDecimals } = this.resolveExportDimensionFormatSettings({
                        format,
                        valueType
                    }));
                    if (isFlat) {
        ...
        async composeCreateReportPivotTotal(fields, { dimensions, metrics, filters }) {
            ...
            const conditions = uniqWith(
                fields.flatMap((field) => {
                    ...
                    const { type } = this.resolveExportNumberFormatSettings(format);
            ...
            const composeTotal = ({ totalSettings, query, position }) => {
                ...
                const { type, decimals } = this.resolveExportNumberFormatSettings(totalFormat);
