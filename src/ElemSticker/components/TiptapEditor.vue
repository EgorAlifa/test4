<template>
    ...
    >
        ...
        <ui-editor-content :editor="editor" class="tiptap-editor__content" />
    </div>
</template>
...
export default {
    ...
    computed: {
        ...
        currentTextColor() {
            if (this.editor == null) { return '#000000'; }
            const { color } = this.editor.getAttributes('textStyle');
            return color || '#000000';
        },

        currentHighlightColor() {
            if (this.editor == null) { return '#ffff00'; }
            const { color: highlight } = this.editor.getAttributes('highlight');
            return highlight || '#ffff00';
        },

        currentFontSize() {
            if (this.editor == null) { return DEFAULT_FONT_SIZE; }
            const { fontSize } = this.editor.getAttributes('textStyle');
