...
import { ValueObject, store as globalStore } from '@goodt-wcore/managers';
...
export default {
    ...
    computed: {
        ...
        storeWatchVars() {
            const { slots = [], awaitVariableModeVariables = [], awaitStoreFilter: isAwaitStoreFilter } = this.props;
            const slotRulesStoreVars = slots
                .flatMap(({ rules = [] }) => rules)
                .flatMap((conditions = []) => conditions)
                .map((conditionStr) => SlotCondition.fromString(conditionStr))
                .filter((condition) => condition != null)
                .flatMap(({ a, b, type, typeForLeftField }) => [
                    typeForLeftField === SlotConditionType.STORE ? a : null,
                    type === SlotConditionType.STORE ? b : null
                ])
                .filter((key) => key != null && key !== '')
                .map((key) => String(key));

            const baseVars = [Vars.SLOT_NAMES, ...slotRulesStoreVars, ...this.$storeWatchHandlerVars];
            const varsResolved = isAwaitStoreFilter ? [...baseVars, ...awaitVariableModeVariables] : baseVars;

            return [...new Set(varsResolved.filter((key) => key != null && key !== '').map((key) => String(key)))];
        },
        /**
         * @return {string[]}
         */
        ...
        slotNamesToShow() {
            ...
            return [names]
                .flat()
                .map((name) => this.resolveStoreValue(name))
                .flat()
        ...
        isAllowedLoadData() {
            ...
            const storeState = this.resolveStoreStateForQueryFilters();

            if (isAwaitStoreFilter && awaitVariableModeVariables.length === 0) {
                return Object.keys(storeState).some((key) => key !== Vars.SLOT_NAMES);
            }

            return awaitVariableModeVariables.every((variable) => this.getStoreValue(variable) != null);
        }
    },
    computedEditor: {
        /**
    ...
    methods: {
        ...
        resolveStoreState(state = {}) {
            return Object.entries(state).reduce(
                (acc, [key, value]) => ({ ...acc, [key]: this.resolveStoreValue(value) }),
                {}
            );
        },
        resolveStoreStateForQueryFilters() {
            return omit(
                {
                    ...this.resolveStoreState(this.globalStoreState),
                    ...this.resolveStoreState(this.$storeState)
                },
                this.metrics
            );
        },
        resolveStoreValue(value) {
            if (value == null) {
                return value;
            }
            const result = ValueObject.getValue(value);
            if (
                result === value &&
                typeof value === 'object' &&
                !Array.isArray(value) &&
        ...
        async fetchData(forceUpdate = false) {
            const { vm } = this;
            const resolvedStoreState = this.resolveStoreStateForQueryFilters();
            this.isLoadingData = true;
            try {
                const changedRequests = await this.$storeApplyQueryFilters(resolvedStoreState);
        ...
        getStoreValue(key) {
            ...
            const globalState = this.globalStoreState ?? {};
            if (key in state) {
                return this.resolveStoreValue(state[key]);
            }
            if (key in globalState) {
                return this.resolveStoreValue(globalState[key]);
            }
            return null;
        }
    },
