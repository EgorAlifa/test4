    ...
    <div class="condition-item">
        <template v-if="value">
            <span
                ...
                :id="aListId"
                class="condition-item__badge badge text-small w-100"
                :title="a"
                @click="toggleAList">
                {{ a ? a : '_' }}
            </span>
...
export default {
    ...
    computed: {
        ...
        aOptions() {
            return this.metricDimensions.map((value) => ({ label: value, value }));
        },
        opOptions() {
...
.condition-item {
    display: grid;
    gap: 0.25rem;
    grid-template-columns: 2fr auto 1fr 2fr auto auto;
    align-items: center;
    &__badge {
