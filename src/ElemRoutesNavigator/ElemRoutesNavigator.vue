<template>
    <w-elem :placeholder="$placeholder">
        <div v-if="isReady" class="routes-navigator-container" :style="containerStyle">
            ...
            <div
                v-if="!isPlayerMode && routes.length === 0 && props.showHint"
                class="editor-hint"
                title="Сейчас показываются тестовые данные для настройки дизайна. Для перехода на реальные данные запустите проект в плеере."
            >
                💡 Тестовый режим
            </div>
