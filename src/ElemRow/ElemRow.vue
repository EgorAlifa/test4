...
export default {
    ...
    methods: {
        onClick(event) {
            ...
            const subRowEl = this.$el.querySelector('[data-slot="sub-row"]');
            if (subRowEl && subRowEl.contains(event.target)) {
                return;
            }
        ...
        onActionsSend(event) {
            ...
            const subRowEl = this.$el.querySelector('[data-slot="sub-row"]');
            const isFromSubRow = subRowEl != null && subRowEl.contains(event.target);

            /*
             * из строки сначала сажаем выбранную строку, потом
             * посылаем сформированные заранее экшены
             */
            if (!isFromSubRow && (shouldSelectRow || props.canSelectRow)) {
                this.setSelectedRow(rowData);
            }
