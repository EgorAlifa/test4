...
export default {
    ...
    computed: {
        ...
        varBindingsSchema() {
            const { elem } = this;
            return elem?.meta?.varBindings || this.descriptor?.varBindings || null;
        },
        ...
        currentPreset() {
            ...
            const listenKeys = Object.keys(state.listen || {}).filter(key => state.listen[key]?.enabled);
            const writeKeys = Object.keys(state.write || {}).filter(key => state.write[key]?.enabled);
            ...
            const hasAllDefaults = defaultEnabledKeys.every(key => listenKeys.includes(key));
