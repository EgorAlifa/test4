...
export default {
    ...
    computed: {
        ...
        lineHeightString: {
            get() {
                const lineHeight = this.props.lineHeight || { size: 1.5, unit: '' }; // eslint-disable-line no-magic-numbers
                if (typeof lineHeight === 'object') {
                    return `${lineHeight.size}${lineHeight.unit}`;
                }
                return lineHeight;
            },
            set(val) {
        ...
        letterSpacingString: {
            get() {
                const letterSpacing = this.props.letterSpacing || { size: 0, unit: 'px' };
                if (typeof letterSpacing === 'object') {
                    return `${letterSpacing.size}${letterSpacing.unit}`;
                }
    ...
    methods: {
        ...
        toggleDecoration(decoration) {
            ...
            const decorations = current.split(' ').filter(dec => dec !== '');
