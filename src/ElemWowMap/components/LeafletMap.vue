...
import '../utils/L.Mask'; // Leaflet.Mask plugin
...
export default {
    ...
    methods: {
        ...
        initializeMap() {
            ...
            // Create map instance
            this.map = LeafletLib.map(this.$refs.mapContainer, {
                center: this.center,
                zoom: this.zoom,
            ...
            LeafletLib.control.zoom({ position: 'bottomright' }).addTo(this.map);
        ...
        initializeCanvasLayer() {
            ...
            this.canvasLayer = LeafletLib.canvasLayer({
                pane: 'particlesPane'
            });
        ...
        updateTileLayer(provider) {
            ...
            // Add new tile layer
            this.tileLayer = LeafletLib.tileLayer(config.url, {
        ...
        initializeLasso() {
            ...
            if (this.map == null || LeafletLib.Lasso == null) {
                return;
            }

            try {
                // Initialize Lasso instance (EXACT from map2)
                this.lassoInstance = new LeafletLib.Lasso(this.map, {
                    polygon: {
                        color: '#007bff',
                        // eslint-disable-next-line no-magic-numbers
                        weight: 2,
                ...
                this.map.on('lasso.finished', (event) => {
                    ...
                    if (event.layers && event.layers.length > 0) {
                        bounds = LeafletLib.featureGroup(event.layers).getBounds();
                    }
        ...
        addSelectionMarker({ id, lat, lng }) {
            ...
            if (this.selectionMarkers.has(id)) {
                ...
                const marker = LeafletLib.circleMarker([lat, lng], {
                    radius: 1,
                    // eslint-disable-next-line no-magic-numbers
                    opacity: 0.001,
                    // eslint-disable-next-line no-magic-numbers
        ...
        updateLassoPolygon() {
            ...
            if (this.isLassoFilterApplied && this.activeLassoPath && this.activeLassoPath.length > 0) {
                ...
                this.lassoAreaDisplayPolygon = LeafletLib.polygon(this.activeLassoPath, {
                    fill: false,
                    color,
                    // eslint-disable-next-line no-magic-numbers
                    weight: 2,
