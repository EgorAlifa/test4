...
```
src/ElemMyChart/
├── descriptor.js           # Props definition + meta registration
├── ElemMyChart.vue         # Main Vue component
├── panels/
│   ├── index.js            # Panel exports (lazy loaded)
│   ├── DatasetPanelMixin.js  # Auto-watcher for metrics/dimensions
│   ├── config.js           # Static options (dropdowns, formats)
│   ├── DimensionsPanel.vue
│   ├── MetricsPanel.vue
│   └── OptionsPanel.vue
├── utils/
│   ├── constants.js        # Default factories (series, axis, tooltip)
│   └── index.js            # Helpers (formatting, aggregation, sorting)
├── styles/
│   ├── css-vars.js         # Theme variable definitions
...
Widgets surface configuration through lazy-loaded panels registered in `panels/index.js`. Group panel responsibilities into dataset bindings, metric styling, axis tuning, and general options—the palette already offers specialised UI primitives (`ui-select`, `ui-switch`, `ui-input-units`, rich tooltips) that should always be reused instead of bespoke controls.
...
```vue
<!-- ✅ GOOD: Color picker for all colors -->
<ui-input-cp prop="activeColor">Цвет активных сегментов</ui-input-cp>
<ui-input-cp prop="valueFontColor">Цвет шрифта</ui-input-cp>

<!-- ❌ BAD: Plain input for colors -->
...
<ui-input-cp prop="valueFontColor">Цвет шрифта</ui-input-cp>
<ui-input-auto prop="valueFontFamily">Шрифт</ui-input-auto>
<ui-input-units
    col-size="6-12"
    min="0"
...
- ✅ Font sizes (valueFontSize, titleFontSize, labelFontSize, etc.)
- ✅ Bar/chart dimensions (barHeight, barWidth, gaugeSize, chartHeight, etc.)
- ✅ Spacing/gaps (barGap, itemSpacing, rowGap, columnGap, etc.)
- ✅ Padding/margins (containerPadding, itemPadding, margin, etc.)
- ✅ Any other size-related setting that benefits from responsive units
...
// ❌ NEVER use plain Number type for dimensions:
barHeight: {
    type: Number,  // WRONG!
    default: 128,
    label: 'Высота бара (px)'
}
```
...
- ✅ UI shows default values correctly (not empty fields)
- ✅ Changes to size/unit update immediately and reactively
- ✅ All units (px, rem, em, %) are properly converted to pixels using `unit2PxMixin`
- ✅ Support for CSS variables (e.g., `var(--custom-size)`)
- ✅ Consistent type safety across the widget
- ✅ Responsive design capabilities with relative units (rem, em)
- ✅ Uniform API for all dimension-related settings
...
<ui-has-panel>
    ...
    <template #panel>
        <ui-panel :groups="[{ name: 'Настройки заголовка', slot: 'title' }]">
            <template #title>
                <ui-container>
                    <ui-input prop="title" placeholder="Текст заголовка"></ui-input>
                    <div class="form-label form-label-small">Шрифт</div>
                    <ui-input-cp prop="titleFontColor">Цвет шрифта</ui-input-cp>
                    <ui-input-auto prop="titleFontFamily">Шрифт</ui-input-auto>
                    <ui-input-units
                        col-size="6-12"
                        min="0"
                        :units="FontSizeFirstPxUnits"
...
- ✅ "Метрики" (Metrics) instead of "Источник данных" (Data Source)
- ✅ "Настройки виджета" (Widget Settings) instead of "Опции" (Options)
- ✅ Group logically: Metrics → Design → Layout
...
<!-- ❌ BAD: Excessive spacing with dividers -->
<ui-container>
...
1. **DO NOT use `watch` on `props.customStyles`** - This creates a reactive loop:
   - User types → updates `localStyles`
   - `@input` updates `props.customStyles`
   - Watch triggers → resets `localStyles` from props
   - **Field gets cleared!** 😱
...
- **Panel Name**: Use creative, user-friendly names like "Я дизайнер" (I'm a Designer) instead of "Custom CSS"
...
- ❌ Don't add `backgroundColor` if platform handles it
- ❌ Don't add `borderColor` or `borderWidth` if platform provides it
- ✅ Do add widget-specific colors (e.g., gauge segment colors, chart series colors)
- ✅ Do add theme selection if widget needs internal color coordination
...
The Insight Data Platform supports **complex dimensions** — hierarchical structures where a dimension has child levels (e.g., `product` → `product_category` → `product_name`). When a dataset is configured with complex dimensions, the `$drilldown` service provides drill-down state for each dimension that has child levels.
...
>
    ...
    >
        ...
        <span v-if="isDrillableCell(column) && !isTextColumn(column)" class="drilldown-icon">🔍</span>
    </div>
</div>
```
...
2. **Apply defaults to drilldown state**: Spread actual state AFTER defaults to prevent `undefined` values:
   ...
   // ❌ BAD - canPush could be undefined
...
5. **Filter only drillable dimensions**: Not all dimensions in `props.dimensions` are drillable — only those with `$drilldown.state[dim]` configured.
...
computed: {
    ...
    emptyStateMessage() {
        if (!this.hasDatasetConfigured) {
            return 'Настройте источник данных и добавьте измерения или метрики';
        }
        return 'Добавьте измерения и метрики для отображения таблицы';
    }
}
```
...
```css
width: clamp(100px, 20%, 160px);  /* ✅ Good: 20% of container */
width: clamp(100px, 20vw, 160px); /* ❌ Bad: 20% of viewport */
```
...
<!-- ❌ BAD: Fixed size breaks responsiveness -->
...
```javascript
// ✅ GOOD: Safe property access with proper fallbacks
valueStyle() {
    const isDark = this.props.theme === 'dark';
    const defaultColor = isDark ? '#ffffff' : '#0a0a0a';
...
// ❌ BAD: Unsafe destructuring without fallbacks
valueStyle() {
...
```javascript
// ✅ GOOD: Scales with container size
textPositionStyle() {
    return {
...
// ❌ BAD: Fixed pixel values break at different sizes
textPositionStyle() {
    return {
...
```vue
<!-- ❌ BAD: SVG text has font rendering issues -->
<svg>
    <text x="17" y="16.5" :style="textStyle">{{ value }}</text>
</svg>

<!-- ✅ GOOD: HTML text with absolute positioning -->
<div class="relative">
    <svg><!-- shapes only --></svg>
...
```javascript
// ✅ GOOD: Smooth animation tied to browser refresh rate
this.animationFrameId = requestAnimationFrame(animate);

// ❌ BAD: Choppy animation, not synced with browser
...
```javascript
// ✅ GOOD: Cleanup on component destruction
beforeDestroy() {
    if (this.animationFrameId == null) {
        return;
    }
...
// ❌ BAD: Animation continues after component destroyed
beforeDestroy() {
    // Missing cleanup
}
```
...
// ❌ BAD: Multiple animations running simultaneously
animateValue(from, to) {
...
```javascript
// ✅ GOOD: Skip animation when disabled
if (this.props.animation === false) {
    this.animatedValue = to;
    return;
}

// ❌ BAD: Using animation flag incorrectly
...
```vue
<!-- ✅ GOOD: CSS transitions for color, JS for value -->
<path
...
// ❌ BAD: Single letter parameter (linter error)
'ease-out': (t) => t * (2 - t)
```
...
```javascript
// ✅ GOOD: Disable linter for magic number in default
valueFontSize: {
    type: Object,
    default: () => ({ size: 40, unit: 'px' }), // eslint-disable-line no-magic-numbers
    label: 'Размер значения'
...
// ❌ BAD: Magic number without comment causes linter error
default: () => ({ size: 40, unit: 'px' })
```
...
```javascript
// ✅ GOOD: Named constant for clarity
computed: {
    valueStyle() {
        const defaultValueSize = 40;
...
```javascript
// ✅ GOOD: Arrays end with 's', constants are PascalCase
export const GaugeRadiuses = [
    { label: 'Малое', value: 'sm' },
    { label: 'Среднее', value: 'md' },
    { label: 'Большое', value: 'lg' }
];
...
// ❌ BAD: Unused data causes linter error
data() {
    return {
        options,
...
```javascript
// ✅ GOOD: Destructure nested properties
const { size = 40, unit = 'px' } = this.props.valueFontSize || {};
const fontSize = size && unit ? `${size}${unit}` : '40px';

// ❌ BAD: Long chains trigger linter warning
...
<!-- ❌ BAD: Template literal in template -->
...
```javascript
// ❌ BAD: All widget instances share the same object
props: {
    legend: {
        type: Object,
...
// ✅ GOOD: Each instance gets a fresh copy
props: {
    legend: {
        type: Object,
        default: () => ({ show: false, position: 'top' })
...
```javascript
// ❌ BAD: Palette UI updates but widget doesn't redraw
methods: {
    changeLegend() {
...
// ✅ GOOD: Always notify after mutations
methods: {
    changeLegend() {
        this.props.legend.show = true;
...
1. **Module Architecture**: Descriptor → Panels → Component → Utils → Styles
